...
namespace Dazoot\Newsman\Helper\Newsman;

use Dazoot\Newsman\Helper\Newsman\ClientEx\NewsmanException;
...
class Client
{
    ...
    protected function _detectTransport()
    {
        ...
        if (is_null($this->call_type)) {
            if (@class_exists("Zend_XmlRpc_Client")) {
                ...
                throw new NewsmanException("No extensions found for the Newsman Api Client. Requires either Zend_XmlRpc_Client or xmlrpc_encode extension for RPC calls or Zend_Http_Client or CURL extension for REST calls.");
            }
    ...
    {
        if (is_null($this->method_namespace)) /* throw new NewsmanException("No namespace defined"); */
        ...
        if ($this->call_type == "rpc") {
           ...
           /* throw new NewsmanException("Unknown call type: $this->call_type"); */
        }
    ...
    public function sendRequestRpc($api_method, $api_params)
    {
        ...
        if ($this->transport == "zend_xmlrpc_client") {
           ...
           /* throw new NewsmanException("Zend_XmlRpc_Client or xmlrpc_encode extension are required to make the POST RPC call."); */
        }
    ...
    public function sendRequestRest($api_method, $params)
    {
        ...
        if ($this->transport == "zend_http_client") {
            ...
            // should never reach this unless setTransport is called with weird transport name
          /*  throw new NewsmanException("Zend_Http_Client or curl extension are required to make the POST REST call."); */
        }
        ...
        if ($this->output_format == "json") {
            ...
            if ($this->output_format == "ser") {
               ...
               /* throw new NewsmanException("Unknown serialization format: $this->output_format"); */
            }

        // check for errors
        if (is_array($ret)) {
            if (isset($ret["err"]) && ($ret["err"] === "true" || $ret["err"] === true)) {
               /* throw new NewsmanException($ret["message"], $ret["code"]); */
            }
        }
    ...
    protected function _rpc_Zend($url, $params)
    {
        ...
        try {
            ...
            /* throw new NewsmanException($e->getMessage()); */
        }
        return $ret;
    }
    ...
    protected function _rpc_xmlrpc($url, $params)
    {
        ...
        if (!$response) {
           /* throw new NewsmanException("Could not make the RPC call"); */
        }

        if (is_array($response) && xmlrpc_is_fault($response)) {
            /* throw new NewsmanException("Error on the RPC call: " . $response["faultString"]); */
        }
